...
2. **Implementación de MongoDB y Mongoose:** 
   ...
   - **Variables de Entorno:** Implementamos el archivo `.env` para ocultar la URL de conexión a la base de datos, protegiendo credenciales sensibles.
...
5. **Autenticación "Stateless":** Respetando la teoría de las APIs REST (sin estado), implementamos un sistema de registro y login visual. Este sistema valida el acceso mediante operaciones CRUD puras sobre la colección de Usuarios, sin utilizar herramientas avanzadas de manejo de sesiones, cumpliendo estrictamente con los lineamientos actuales de la cátedra.
6. **Middlewares:** Mantenemos nuestro logger de auditoría para cada petición HTTP y conservamos el manejo de errores 404 para rutas inexistentes.
...
Como la base de datos local de MongoDB estará vacía en tu equipo, necesitás poblarla ejecutando el siguiente script en la terminal para cargar los catálogos base de Clientes (Sucursales) y Productos:
...
El servidor estará corriendo en http://localhost:3000, redirigiendo automáticamente a la pantalla de Login.
